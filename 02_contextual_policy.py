...
class contextual_bandit():
    def __init__(self):
        self.state = 0
        # 밴딧들의 손잡이 목록, 각 밴딧은 각각 손잡이 4, 2, 1이 최적이다.
        self.bandits = np.array([[0.2,0,-0.0,-5],[0.1,-5,1,0.25],[-5,5,5,5]])
        self.num_bandits = self.bandits.shape[0]
        self.num_actions = self.bandits.shape[1]
        
    def getBandit(self):
        # 각각의 에피소드에 대한 랜덤한 상태를 반환
...
class agent():
    def __init__(self, lr, s_size,a_size):
        ...
        self.state_in= tf.compat.v1.placeholder(shape=[1],dtype=tf.int32)
        ...
        self.loss = -(tf.compat.v1.log(self.responsible_weight)*self.reward_holder)
...
with tf.compat.v1.Session() as sess:
    ...
    while i < total_episodes:
        ...
        # 주어딘 밴딧에 대해 액션을 취한 데 대한 보상을 얻는다.
